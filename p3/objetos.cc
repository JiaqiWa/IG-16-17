...
_cubo& _cubo::operator=(const _cubo &obj)
{
...
_revolucion& _revolucion::operator=(const _revolucion &obj)
{
        vertices=obj.vertices;
        caras=obj.caras;
}
...
_grua::_grua(_cubo &cub, _revolucion &cilindr, float a, float b,float ang, float g1, float g2, float g3)
{
        cubo=cub;
        cilindro=cilindr;
        alfa=a;
...
void _grua::dibuja(string m)
{
        modo=m;
        glRotatef(beta,0,1,0);
        glTranslatef(alfa,0,0);
        parteInferior();
...
void _grua::parteSuperior()
{
        //cilindro giratorio
        glPushMatrix();
        glScalef(2,1,2);
        if (modo == "P")
                cilindro.draw_puntos(1,0,1,5);
        else if (modo == "L")
                cilindro.draw_aristas(0,1,1,4);
        else if (modo == "S")
                cilindro.draw_solido(2,0,1);
        else if (modo == "A")
                cilindro.draw_solido_ajedrez(1,1,0,0,0,0.3);
...
void _grua::brazosCuerdaCaja()
{
        ...
        //Brazo Horizontal, cuerda y caja


}

void _grua::brazoHcuerdaCaja()
{
...
void _grua::cuerdaCaja()
{

        glPushMatrix();
        float tmp = abs(13-grado3);
        float tmp2 = abs(grado3);
        glTranslatef(9,-13+tmp2,0);
        glScalef(0.05,tmp,0.05);
        if (modo == "P")
                cilindro.draw_puntos(1,0,1,5);
        else if (modo == "L")
                cilindro.draw_aristas(0,1,1,4);
...
{
        glPushMatrix();
        glTranslatef(8.5,2.5,1.5);
        luz();
        glPopMatrix();

        glPushMatrix();
        glTranslatef(8.5,2.5,4.5);
        luz();
        glPopMatrix();
}
